...
pubDatetime: 2023-02-05T20:22:00Z
title: "Scaling images to every device in Astro"
...
If you want to immediately implement it, you can skip to the end of the article. ADD LINK
...
It tries to solve the problem by extracting the display width of images on the client-side.
However, this means that the right image variant can only be selected after the page has already loaded.
...
If you are on a strict time budget, a library like lazysizes is definitely a great option.
